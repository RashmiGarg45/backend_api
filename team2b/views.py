...
class AnqgoldrewardsAPI(APIView):
    ...
    def get(self, request):
        ...
        query = Anqgoldrewards.objects.filter(used_at=None).order_by('-created_at')[0:50].first()
        ...
        if setUsed:
            query = Anqgoldrewards.objects.filter(id=data.get('user_id')).update(used_at=datetime.now().strftime('%Y-%m-%d %H:%M:%S'), channel=channel, network=network, offer_id=offer_id)
        return Response({
            'body':data,
        })
    
    def post(self, request):
        query = Anqgoldrewards.objects.order_by('-id').first()
