...
class igpAPI(APIView):
    ...
    def get(self, request):
        ...
        query = IGP.objects.latest('created_at')
        
        data = {
                'user_id':query.id,
        }
        if setUsed:
            query = IGP.objects.filter(id=data.get('user_id')).update(used_at=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        return Response({
            'body':data,
        })
