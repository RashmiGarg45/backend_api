...
class UnderarmouruidAPI(APIView):
    def put(self, request):
        query = Underarmouruid()
        query.campaign_name = request.data.get('camp_name','underarmourauto')
        query.id = request.data.get('user_id')
        query.used_at = None
        query.save()
    ...
    def get(self, request):
        ...
        query = Underarmouruid.objects.latest('created_at')
        
        data = {
                'user_id':query.id,
        }
        if setUsed:
            query = Underarmouruid.objects.filter(id=data.get('user_id')).update(used_at=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        return Response({
            'body':data,
        })