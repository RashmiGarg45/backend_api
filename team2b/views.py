...
class GenericScriptFunctions(APIView):
    def get(self, request):
        tablesDict = {
            ...
            'gomcdoauto/mcdot2ios': GomcdOrderIds,
...
class Lotter38API(APIView):
    def put(self, request):
        query = Lotter_a()
        query.campaign_name = request.data.get('camp_name','Lotter_a')
        query.id = request.data.get('order_id')
        query.used_at = None
        query.save()
        return Response({
    ...
    def get(self, request):
        ...
        query = Lotter_a.objects.latest('created_at')
        
        data = {
                'order_id':query.id,
        }
        if setUsed:
            query = Lotter_a.objects.filter(id=data.get('order_id')).update(used_at=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        return Response({
            'body':data,
        })
...
class Lotter69API(APIView):
    def put(self, request):
        query = Lotter_a()
        query.campaign_name = request.data.get('camp_name','Lotter_b')
        query.id = request.data.get('order_id')
        query.used_at = None
        query.save()
        return Response({
    ...
    def get(self, request):
        ...
        query = Lotter_b.objects.latest('created_at')
        
        data = {
                'order_id':query.id,
        }
        if setUsed:
            query = Lotter_b.objects.filter(id=data.get('order_id')).update(used_at=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        return Response({
            'body':data,
        })
